...
def voice_choice_keyboard() -> InlineKeyboardMarkup:
    buttons = [
        [InlineKeyboardButton("👨 សំឡេងប្រុសទាំងអស់", callback_data=VOICE_MALE)],
        [InlineKeyboardButton("👩 សំឡេងស្រីទាំងអស់", callback_data=VOICE_FEMALE)],
        [InlineKeyboardButton("👫 ឆ្លាស់គ្នា (ប្រុស+ស្រី)", callback_data=VOICE_ALTERNATE)],
    ]
    return InlineKeyboardMarkup(buttons)
...
async def voice_choice_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    labels = {
        VOICE_MALE: "សំឡេងប្រុសទាំងអស់ 👨",
        VOICE_FEMALE: "សំឡេងស្រីទាំងអស់ 👩",
        VOICE_ALTERNATE: "ឆ្លាស់គ្នា (ប្រុស+ស្រី) 👫",
    }
    await query.edit_message_text(
...
async def handle_srt2audio(update, context, input_path, ext, work_dir):
    ...
    voice_label = {
        "male": "សំឡេងប្រុស 👨",
        "female": "សំឡេងស្រី 👩",
        "alternate": "ឆ្លាស់គ្នា 👫",
    }.get(voice_mode, "ឆ្លាស់គ្នា 👫")
