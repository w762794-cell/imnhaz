...
async def handle_transcribe(update, context, input_path, ext, work_dir):
    ...
    status_msg = await update.message.reply_text(
        "កំពុងស្តាប់ និងសរសេរជាអត្ថបទ... 📝 (0 វិនាទី)"
    )
    ...
    task = asyncio.create_task(
        asyncio.to_thread(
            transcribe_to_srt, input_path, out_srt, work_dir, None, model_size
        )
    )
        ...
        try:
            ...
            try:
                await status_msg.edit_text(
                    f"កំពុងស្តាប់ និងសរសេរជាអត្ថបទ... 📝 ({elapsed} វិនាទី, "
                    "នៅតែកំពុងដំណើរការ សូមអត់ធ្មត់)"
                )
            except Exception:
                pass
